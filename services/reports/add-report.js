...
let reportCreate = function (key, cardId, parent, userId, name, birth_date, height, weight, vaccination) {
  ...
  async function main() {
    try {
      ...
      const result = await contract.createTransaction('createReport')
        .submit(key, JSON.stringify(reportData), cardId, parent)
      ;
